...
import { solanaNetworkConfigs, getAllSolanaEndpoints, type SolanaNetworkConfig, type SolanaRpcEndpoint } from '@/config/solana/rpc-endpoints';
...
interface SolanaRpcEndpointConfigProps {
  selectedNetwork: SolanaNetworkConfig | null;
  selectedEndpoint: SolanaRpcEndpoint | null;
  onNetworkChange: (network: SolanaNetworkConfig) => void;
  onEndpointChange: (endpoint: SolanaRpcEndpoint) => void;
}

export default function SolanaRpcEndpointConfig({
  selectedNetwork,
  selectedEndpoint,
  onNetworkChange,
  onEndpointChange,
}: SolanaRpcEndpointConfigProps) {
  ...
  useEffect(() => {
    if (selectedNetwork) {
      const endpoints = getAllSolanaEndpoints(selectedNetwork.network);
  ...
  const testLatency = async (endpoint: SolanaRpcEndpoint) => {
    const startTime = Date.now();
    try {
      await sendRpcRequest(endpoint.url, 'getHealth', []);
  ...
  const allEndpoints = selectedNetwork
    ? getAllSolanaEndpoints(selectedNetwork.network)
    : [];
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Select Solana network and RPC endpoint</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) => {
              const network = solanaNetworkConfigs.find((n) => n.network === value as any);
              if (network) {
          ...
          >
            ...
            <SelectContent>
              {solanaNetworkConfigs.map((network) => (
