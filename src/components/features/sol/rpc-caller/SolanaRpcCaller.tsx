...
import { solanaNetworkConfigs, type SolanaNetworkConfig, type SolanaRpcEndpoint } from '@/config/solana/rpc-endpoints';
import { solanaRpcMethods, getSolanaRpcMethod } from '@/config/solana/rpc-methods';
import { type RpcMethod } from '@/components/features/common/rpc-caller/types';
import SolanaRpcEndpointConfig from './SolanaRpcEndpointConfig';
...
const STORAGE_KEYS = {
  SELECTED_NETWORK: 'solana_rpc_selected_network',
  CALL_HISTORY: 'solana_rpc_call_history',
};

export default function SolanaRpcCaller() {
  const [selectedNetwork, setSelectedNetwork] = useState<SolanaNetworkConfig | null>(null);
  const [selectedEndpoint, setSelectedEndpoint] = useState<SolanaRpcEndpoint | null>(null);
  ...
  useEffect(() => {
    try {
      ...
      if (savedNetwork) {
        const network = solanaNetworkConfigs.find((n) => n.network === savedNetwork);
        if (network) {
          setSelectedNetwork(network);
        }
      } else {
        // Default to mainnet-beta
        const mainnet = solanaNetworkConfigs.find((n) => n.network === 'mainnet-beta');
        if (mainnet) {
  ...
  const handleHistoryItemClick = (method: string, params: unknown[]) => {
    const methodConfig = getSolanaRpcMethod(method);
    if (methodConfig) {
  ...
  const categories = ['account', 'block', 'transaction', 'network'] as const;

  return (
    <div className="w-full p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold tracking-tight">Solana RPC API Caller</h1>
        <p className="text-muted-foreground">
          Directly call RPC interfaces of Solana blockchain
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-6">
          ...
          <MethodSelector
            methods={solanaRpcMethods}
            categories={categories}
            getMethod={getSolanaRpcMethod}
