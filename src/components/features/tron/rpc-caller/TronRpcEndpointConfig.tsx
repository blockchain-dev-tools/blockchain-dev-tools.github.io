...
import { tronNetworkConfigs, getAllTronEndpoints, type TronNetworkConfig, type TronRpcEndpoint } from '@/config/tron/rpc-endpoints';
...
interface TronRpcEndpointConfigProps {
  selectedNetwork: TronNetworkConfig | null;
  selectedEndpoint: TronRpcEndpoint | null;
  onNetworkChange: (network: TronNetworkConfig) => void;
  onEndpointChange: (endpoint: TronRpcEndpoint) => void;
}

export default function TronRpcEndpointConfig({
  selectedNetwork,
  selectedEndpoint,
  onNetworkChange,
  onEndpointChange,
}: TronRpcEndpointConfigProps) {
  ...
  useEffect(() => {
    if (selectedNetwork) {
      const endpoints = getAllTronEndpoints(selectedNetwork.network);
  ...
  const testLatency = async (endpoint: TronRpcEndpoint) => {
    const startTime = Date.now();
    try {
      await sendRpcRequest(endpoint.url, 'eth_blockNumber', []);
  ...
  const allEndpoints = selectedNetwork
    ? getAllTronEndpoints(selectedNetwork.network)
    : [];
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Select Tron network and RPC endpoint</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) => {
              const network = tronNetworkConfigs.find((n) => n.network === value as any);
              if (network) {
          ...
          >
            ...
            <SelectContent>
              {tronNetworkConfigs.map((network) => (
