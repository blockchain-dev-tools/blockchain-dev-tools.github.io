...
import { suiNetworkConfigs, type SuiNetworkConfig, type SuiRpcEndpoint } from '@/config/sui/rpc-endpoints';
import { suiRpcMethods, getSuiRpcMethod } from '@/config/sui/rpc-methods';
import { type RpcMethod } from '@/components/features/common/rpc-caller/types';
import SuiRpcEndpointConfig from './SuiRpcEndpointConfig';
...
const STORAGE_KEYS = {
  SELECTED_NETWORK: 'sui_rpc_selected_network',
  CALL_HISTORY: 'sui_rpc_call_history',
};

export default function SuiRpcCaller() {
  const [selectedNetwork, setSelectedNetwork] = useState<SuiNetworkConfig | null>(null);
  const [selectedEndpoint, setSelectedEndpoint] = useState<SuiRpcEndpoint | null>(null);
  ...
  useEffect(() => {
    try {
      ...
      if (savedNetwork) {
        const network = suiNetworkConfigs.find((n) => n.network === savedNetwork);
        if (network) {
          setSelectedNetwork(network);
        }
      } else {
        // Default to mainnet
        const mainnet = suiNetworkConfigs.find((n) => n.network === 'mainnet');
        if (mainnet) {
  ...
  const handleHistoryItemClick = (method: string, params: unknown[]) => {
    const methodConfig = getSuiRpcMethod(method);
    if (methodConfig) {
  ...
  const categories = ['account', 'object', 'transaction', 'network'] as const;

  return (
    <div className="w-full p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold tracking-tight">Sui RPC API Caller</h1>
        <p className="text-muted-foreground">
          Directly call RPC interfaces of Sui blockchain
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-6">
          ...
          <MethodSelector
            methods={suiRpcMethods}
            categories={categories}
            getMethod={getSuiRpcMethod}
            selectedMethod={selectedMethod}
            customMethodName={customMethodName}
            placeholder="e.g., sui_getBalance"
